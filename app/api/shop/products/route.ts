...
// Get user from JWT token (Authorization Bearer or cookie, for mobile + web)
async function getUserFromToken(request: NextRequest) {
  try {
    ...
    // Support both userId (login-direct) and id
    if (decoded.userId != null && decoded.id == null) decoded.id = decoded.userId;
    return decoded;
  } catch (error) {
    return null;
  }
