...
import { ArrowLeft, Share2, Heart, ExternalLink, Users, Target } from 'lucide-react';
...
export default function WishlistDetailPage() {
  ...
  const checkPaymentStatus = async (depositId: string) => {
    try {
      console.log('Checking payment status for depositId:', depositId);
      ...
      if (result.success && result.isCompleted) {
        ...
        setTimeout(async () => {
          try {
            console.log('Attempting secure token generation...');
            ...
            if (tokenResult.success) {
              console.log('Token generated successfully, redirecting with secure token...');
              setProcessingPayment(false);
              setTimeout(() => {
        ...
        console.log('Payment failed or error:', result);
        setProcessingPayment(false);
        setShowTimeout(true);
      }
    } catch (error) {
  ...
  const hashtags = wishlistItem.hashtags ? wishlistItem.hashtags.split(',').map(tag => tag.trim()) : [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link
                href="/login"
                className="px-4 py-2 rounded-lg font-semibold text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 transition-all duration-200"
              >
                Log in
              </Link>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
            ...
            <div className="bg-white rounded-2xl lg:rounded-3xl shadow-xl p-4 sm:p-6 lg:p-8">
              ...
              <div className="mb-6 lg:mb-8">
                <div className="relative">
                  <div className="w-full h-64 sm:h-80 lg:h-96 bg-gray-100 rounded-xl lg:rounded-2xl overflow-hidden mb-4 flex items-center justify-center">
              ...
              <div className="flex flex-wrap gap-2 mb-4 lg:mb-6">
                <span className="px-2 py-1 sm:px-3 rounded-full text-xs sm:text-sm font-semibold bg-orange-100 text-orange-700 border border-orange-200">
                  {wishlistItem.category}
                </span>
                {hashtags.slice(0, 3).map((tag, index) => (
                  <span
                    key={index}
                    className="px-2 py-1 sm:px-3 rounded-full text-xs sm:text-sm font-semibold bg-orange-100 text-orange-700 border border-orange-200"
                  >
                    #{tag}
                  </span>
                ))}
              ...
              {/* Creator Info */}
              <div className="flex items-center space-x-3 sm:space-x-4 mb-4 lg:mb-6 p-3 sm:p-4 bg-gray-50 rounded-xl">
                <div className="relative flex-shrink-0">
                  {creator.avatar_url ? (
                    <Image
                      ...
                      width={48}
                      height={48}
                      className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-orange-200"
                    />
                  ) : (
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-base sm:text-lg">
                      {getInitials(creator.display_name)}
                    </div>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-base sm:text-lg text-gray-900 truncate">{creator.display_name}</h3>
                  <p className="text-sm text-gray-600 truncate">{creator.bio || 'Creator'}</p>
                </div>
              </div>

              {/* Description */}
              <div className="mb-4 lg:mb-6">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-3">About this wish</h2>
              ...
              {wishlistItem.link && (
                <a
                  ...
                  className="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 transition-all duration-200 text-sm sm:text-base"
                >
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl lg:rounded-3xl shadow-xl p-4 sm:p-6 lg:sticky lg:top-24">
              {/* Progress */}
              <div className="mb-4 sm:mb-6">
                <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                  <div className="relative w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0">
                    <svg className="w-16 h-16 sm:w-20 sm:h-20" viewBox="0 0 40 40">
                      <defs>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-sm sm:text-lg font-bold text-gray-900">
                      {progressPercentage}%
                    </div>
                  </div>
                  <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                <span className="inline-flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white border border-orange-200 flex-shrink-0">
                ...
                <span className="font-semibold text-sm sm:text-lg text-[#f97315] min-w-0">
              ...
              <div className="mb-4 sm:mb-6">
                <button
                  onClick={handleShare}
                  className="w-full py-2 sm:py-3 rounded-xl text-white font-bold text-sm sm:text-base flex items-center justify-center gap-2"
              ...
              <div className="space-y-3 sm:space-y-4">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900">Support this wish</h3>
                ...
                <form onSubmit={handleSupportSubmit} className="space-y-3 sm:space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-1 sm:gap-2">
                    {[5000, 10000, 50000].map((amount) => (
                      <button
                        ...
                        className={`px-2 py-2 sm:px-3 rounded-lg border-2 font-semibold text-xs sm:text-sm transition-all ${
                          supportAmount === amount.toString()
                            ? 'border-orange-500 bg-orange-500 text-white'
                            : 'border-orange-500 text-orange-500 hover:bg-orange-50'
                        }`}
                      >
                        <span className="block sm:hidden">TSh {(amount/1000)}k</span>
                  ...
                  <input
                    ...
                    step="100"
                    required
                    className="w-full px-3 py-2 sm:px-4 sm:py-3 rounded-xl border border-gray-300 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-colors text-sm sm:text-base"
                  />
                  ...
                  <button
                    ...
                    className="w-full py-2 sm:py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center text-sm sm:text-base"
                  >
                    {submitting ? (
                      <>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-2xl lg:rounded-3xl shadow-2xl p-6 sm:p-8 max-w-sm w-full text-center relative">
            <button
            ...
            <div className="text-2xl sm:text-3xl font-bold text-green-600 mb-2">Thank You!</div>
            <div className="text-base sm:text-lg mb-4">Your payment was successful</div>
            <div className="text-xl sm:text-2xl font-bold text-orange-500 mb-4">
              {formatCurrency(supportedAmount)}
            </div>
            <div className="text-sm sm:text-base text-gray-600">Your support means everything!</div>
          </div>
        </div>
      )}
    </div>
